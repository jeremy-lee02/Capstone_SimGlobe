...
import { CountryCluster, InputValue, Room, OtherValue, Team, Elasticity, PresetValue, ClusterScore } from "../../typing"
...
function check_input_value(team_id: number, userInput: Array<UserInput>, inputValue: InputValue): UserInput {
    let newInput = {
        name: 0,
        input: inputValue
    }
...
// consumption pre-tax
function consumption_pre_tax_function(country: CountryCluster, newInput: InputValue) {
...
function investment_pre_tax_function(country: CountryCluster, newInput: InputValue) {
    ...
    return country.other_value.investment_pre_tax + Math.abs(country.other_value.investment_pre_tax) * (last_year_Y_percent - r_percent - r + parseFloat(sigmoid.toFixed(2)) * country.preset_value.impact_of_government_debt_on_investment_growth)/100
...
function income_tax_function(newInput: InputValue, consumption_pre_tax: number) {
...
function consumption_function(newInput: InputValue, consumption_pre_tax: number) {
    return consumption_pre_tax * (1 - newInput.vat_rate/100)
}

function import_preTariff_function(country: CountryCluster){
    const a = country.preset_value.initial_autonomous_imports + country.preset_value.portion_of_gdp_as_induced_import * country.other_value.nominal
    const b = (country.other_value.exchage_rate - 100) * country.elasticity.impact_of_fx_rate_on_induced_import/100 + 1
    return a/b
}
...
function gov_tariff_billion_function(importTarif: number, import_value: number){
    return importTarif - import_value
}
function corporate_tax_function(newInput: InputValue, investment_pre_tax: number){
    return investment_pre_tax * newInput.corporate_tax_rate/100
}
function budget_surplus_billion_function(newInput:InputValue, corporate_tax_revanue: number, vat_revanue: number, tariff_revanue:number){
    return vat_revanue + corporate_tax_revanue + tariff_revanue - newInput.government_expenditure_us
}
...
function net_capital_function(newInput: InputValue, elasticity: Elasticity, global_interestRate:number){
    return (newInput.interest_rate - global_interestRate) * elasticity.impact_of_interest_rate_differential_on_capital_flow
}

function exchage_rate_function(export_value: number, import_value: number, trade_balance: number, net_capital:number){
    return 1 - (trade_balance + net_capital)/ (export_value + import_value)

}

function capital_stock_function(investment: number, preset: PresetValue, other_value: OtherValue ){
    const result = other_value.capital_stock - other_value.capital_stock * preset.depreciation/100 + investment
    if(result > 0) return result
    return 0
} 

function supply_function(other_value: OtherValue, capital_growth: number, labor: number, technological: number ){
...
function consumer_price_index_function(round: number, other_value: OtherValue, elasticity: Elasticity, supply: number, demand: number, prevInput: InputValue, newInput: InputValue){
    const demand_over_supply = (((demand / other_value.demand)/(supply / other_value.supply)) - 1) * elasticity.impact_of_supply_and_demand_change_on_inflation + 1
    const f_t_1 = 1 + (other_value.inflation) * (elasticity.impact_of_inflation_expectation_on_inflation/100)
    const interest_rate_calculation = 1 - (newInput.interest_rate - prevInput.interest_rate) * elasticity.impact_of_interest_rate_on_inflation/100
    const final_result = other_value.consumer_price_index * demand_over_supply * f_t_1 * interest_rate_calculation
...
function unemployment_function(other_value: OtherValue, preset_value: PresetValue){
    const condition = other_value.unemployment + 0.25 *(other_value.inflation - preset_value.impact_of_government_debt_on_investment_growth * other_value.real)
    if(condition < 0) return 0
    return other_value.unemployment + 0.25 *(other_value.inflation - preset_value.impact_of_government_debt_on_investment_growth - other_value.real)
}
