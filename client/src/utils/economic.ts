...
function consumption_pre_tax(country: CountryCluster, prevInput: InputValue) {
    ...
    const interestRate_change = country.elasticity.impact_of_interest_rate_on_induced_consumption_change * (country.input_value.interest_rate - prevInput.interest_rate)
    const interest_rate_level =  country.elasticity.impact_of_interest_rate_on_induced_consumption_level * (country.input_value.interest_rate - prevInput.interest_rate)
...
function investment_pre_tax(country: CountryCluster, prevInput: InputValue) {
    const last_year_Y_percent = country.other_value.nominal
    const r_percent = country.elasticity.impact_of_interest_rate_on_induced_investment_change * (country.input_value.interest_rate - prevInput.interest_rate)
...
function updateCountry(room : Room, prevInput: InputValue, countryName: string): Room {
  ...
  updatedRoom.team.forEach(team => {
      if(countryName === team.country.name){
        team.country.cluster.preset_value = {
            ...team.country.cluster.preset_value,
            initial_consumption: consumption_pre_tax(team.country.cluster, prevInput),
            initial_investment: investment_pre_tax(team.country.cluster, prevInput)
        }
...
export function useUpdateRoom (room: Room, prevInput: InputValue, countryName: string) : Room {
    return updateCountry(room, prevInput,countryName)
}
